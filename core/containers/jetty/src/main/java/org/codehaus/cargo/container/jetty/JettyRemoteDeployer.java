...
public class JettyRemoteDeployer extends AbstractRemoteDeployer {
	 ...
	 *  
	 *  - A servlet could be added to provide this functionality (like the tomcat manager)
	 *  but this would require that the servlet is already deployed before the jetty server 
	 *  starts (which is not something we can assume)
	 *  
	 *  - JMX could be used to provide an mbean to provide this functionality, but the jetty
	 *  jmx server is not started by default, nor can we assume this bean will be provided
	 *  when jetty starts.
	 *  
	 *  - Set a home value of the jetty server. This would allow for deployment only to a server
	 *  running on the local machine. 
	 *  
	 *  
	 *  Setting the home value is probably the best option as it covers 1 of the 2 options
	 *  (local machine but not remote machine). The first two options require that the remote
	 *  server be started with special conditions which can't be guaranteed.
	 *  
    ...
    */	
	...
	public JettyRemoteDeployer(RemoteContainer container)
	{
		...
		{
			getLogger().warn("no Jetty home has been provided, assuming the default location for installed containers", this.getClass().getName());
	...
	private void deployArchive(String directory, WAR war)
	{
		fileHandler.copyFile(
	            war.getFile(), fileHandler.append(directory, war.getContext() + ".war"));
	}

	/**
	 * Jetty requires a context file to be created to hot deploy the webapp
	 * @param directory The directory to install the context
	 * @param war The war to deployed
	 */
